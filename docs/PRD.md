...
- **文件名一致性**：无论压缩结果来源于远程 URL 还是二进制数据，最终写入 Logseq 资产目录（或插件沙盒目录）的压缩图片必须复用原图的文件名（含扩展名），仅允许路径发生变化。
...
- **文件名编码处理**：写入本地文件前需对 Markdown 中的 URL 片段进行解码，确保真实文件名与原图保持一致；而在回写 Markdown 链接时需要重新进行 URL 编码，保证 Logseq 渲染正确。
